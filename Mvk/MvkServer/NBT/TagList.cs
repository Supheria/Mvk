    ...
    public class TagList : NBTBase
    {
        ...
        public void Set(int index, NBTBase value)
        {
            if (index >= 0 && index < tagList.Count)
            {
                ...
                {
                    throw new Exception("Добавление несоответствующих типов тегов в список тегов");
                }
                tagList[index] = value;
            }
            else
            {
                throw new Exception("Ииндекс выходит за пределы, чтобы установить тег в списке тегов");
            }
        }
